...
-- 16. PIZZAS WITH SALES ABOVE AVERAGE TOTAL PRICE
select * from high_value_pizzas order by total_sales desc;
...
-- 16. PIZZAS WITH SALES ABOVE AVERAGE TOTAL PRICE
create view high_value_pizzas as select pizza_name, sum(total_price) as total_sales from pizza_sales group by pizza_name having sum(total_price) > ( select avg(total_price) from pizza_sales);

-- 17. CREATE INDEX ON PIZZA_NAME_ID
create index idx_pizza_name_id on pizza_sales(pizza_name_id);
select name from sys.indexes where object_id = OBJECT_ID('pizza_sales');
